...
def benchmark_token_count_chars4(text: str) -> int:
    """Estimate token count: 1 token ≈ 4 characters (OpenAI guideline)"""
    return len(text) // 4


def benchmark_token_count_words(text: str) -> int:
    """Count tokens as words (split by whitespace)"""
    return len(text.split())


def benchmark_token_count_tiktoken_estimate(text: str) -> int:
    """
    Estimate token count using a more sophisticated method:
    - Whitespace-separated words
    - Punctuation counts as separate tokens
    - Numbers and special characters
    This approximates GPT tokenizers better than chars/4
    """
...
def benchmark_token_count_compressed(text: str) -> int:
    """
    Token count based on information density:
    Count meaningful symbols (alphanumeric + operators)
    """
    # Remove whitespace and count meaningful characters
    meaningful = re.sub(r'\s+', '', text)
    return len(meaningful) // 3  # Adjusted ratio for compressed representation
...
def benchmark_parsing_time(format_type: str, text: str, iterations: int = 100) -> float:
    """
    Measure average parsing time for a circuit format.
    
    Args:
        format_type: 'qasm', 'json', or 'quyaml'
        text: String representation of the circuit
    ...
    for _ in range(iterations):
        ...
        try:
            if format_type == 'qasm':
                ...
                # Parse JSON (just parsing, not full circuit reconstruction)
            ...
            # If parsing fails, return infinity
            return float('inf')
